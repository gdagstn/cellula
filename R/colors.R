...
#'     generated these palettes.
#'     
#'     These palettes adapt to protanopy (color vision deficiency) at a severity
#'     of 0.5.
...
#'     generated these palettes.
#'     
#'     These palettes adapt to tritanopy (color vision deficiency) at a severity
#'     of 0.5.
...
#' Qualitative color palette - Pear18
#' 
#' Selection of 18 colors from the Pear36 color palette
#' 
#' @param n numeric, the number of colors to pick
#' @noRd
#' @details This palette is a selection of 18 colors from the original Pear36
#'    palette by user PineappleOnPizza on Lospec (https://lospec.com/palette-list/pear36).
#'    It is not optimized for CVD.

.cpal_qual_pear <- function(n){
  
  if(n < 1) stop("You need to retrieve at least one color")
  if(n > 18) stop("This palette supports at most 18 colors")
  
  c("#4da6ff", "#eb564b", "#8fde5d", "#ffb570", "#4b5bab", "#bd4882",
    "#3ca370", "#8c3f5d", "#473b78", "#ffb5b5", "#ba6156", "#80366b",
    "#7e7e8f", "#ff9166", "#5e315b", "#ffe478", "#ff6b97", "#e36956")[seq_len(n)]
}

#' Qualitative color palette - Polychrome24
#' 
#' Selection of 24 colors from the Polychrome36 color palette
#' 
#' @param n numeric, the number of colors to pick
#' @noRd
#' @details This palette is a subset of 24 colors from the Polychrome 36 palette 
#'     from base R. It is not optimized for CVD. 

.cpal_qual_polychrome <- function(n){
  
  if(n < 1) stop("You need to retrieve at least one color")
  if(n > 24) stop("This palette supports at most 24 colors")
  
  c("#F6222E", "#3283FE", "#FEAF16", "#1CBE4F", "#AA0DFE", "#C4451C", 
    "#2ED9FF", "#FBE426", "#AAF400", "#782AB6", "#B10DA1", "#3B00FB", 
    "#F7E1A0", "#16FF32", "#683B79", "#822E1C", "#BDCDFF", "#B5EFB5", 
    "#1C8356", "#C075A6", "#FA0087", "#1C7F93", "#7ED7D1", "#90AD1C")[seq_len(n)]
}

#' Qualitative color palette - Polychrome24 light
#' 
#' Selection of 24 colors from the Polychrome36 color palette, lightened
#' 
#' @param n numeric, the number of colors to pick
#' @noRd
#' @details This palette is a subset of 24 colors from the Polychrome 36 palette 
#'     from base R, lightened by a factor of 0.4 using the \code{lighten()} function
#'     from \code{colorspace}. It is not optimized for CVD. 

.cpal_qual_polychromelight <- function(n) {
  
  if(n < 1) stop("You need to retrieve at least one color")
  if(n > 24) stop("This palette supports at most 24 colors")
  
  c("#FF7A7D", "#84A7FF", "#FFC78A", "#4CD96D", "#BE77FF", "#F47055", 
    "#91E4FF", "#FFEC79", "#B4FE2C", "#A369E0", "#DD5ACC", "#776BFF", 
    "#FFEAAA", "#8FFF93", "#9870A9", "#BA6357", "#D1DCFF", "#BEF8BE", 
    "#55AB7F", "#E297C7", "#FF79A9", "#59A6BB", "#90E7E1", "#ABC94C")[seq_len(n)]
}

#' Qualitative color palette - Tableau
#' 
#' The original Tableau 10-color palette
#' 
#' @param n numeric, the number of colors to pick
#' @noRd
#' @details This is the 10-color Tableau palette from base R. 
#'    It is not optimized for CVD. 
#' @importFrom grDevices palette.colors    

.cpal_qual_tableau <- function(n){
  
  if(n < 1) stop("You need to retrieve at least one color")
  if(n > 10) stop("This palette supports at most 10 colors")
  
  unname(palette.colors(n = 10, palette = "Tableau 10"))[seq_len(n)]
}
...
#' 
#' Preset continuous color palettes from \code{pals}
#' @noRd
#' @details
#'     These palettes were generated using the \code{pals} package by 
#'     Kevin Wright.
